...
interface uart_bfm;
    ...
    // Send method with intentionally wrong parity bit
    // NEW TASK
    task send_with_bad_parity(
        input byte      data,
        input int       data_len    = 8,
        input real      baud_rate   = 115_200,
        input string    parity_type = "even",
        input bit       stop_bit    = 0
    );
        @(posedge clk);
        ...
        // parity bit intentionally inverted
        if (parity_type == "even") begin
            rxd = ~(^data); // expected for even would be (^data)
            bit_timer(baud_rate);
        end
        else if (parity_type == "odd") begin
            rxd = (^data);  // expected for odd would be ~(^data)
            bit_timer(baud_rate);
        end
